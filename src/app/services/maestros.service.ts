...
export class MaestrosService {
  ...
  public validarMaestro(data: any, editar: boolean) {
    ...
    } else if (data['id_trabajador'].length < 5) {
      error['id_trabajador'] = 'La matrícula debe tener al menos 5 caracteres';
    } else if (data['id_trabajador'].length > 15) {
      error['id_trabajador'] =
        'La matrícula no puede tener más de 15 caracteres';
    }
    ...
    if (!this.validatorService.required(data['fecha_nacimiento'])) {
      ...
      if (edad < 22) {
        error['fecha_nacimiento'] =
          'El maestro debe tener al menos 22 años de edad';
      }
    }
