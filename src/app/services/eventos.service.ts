...
export class EventosService {
  ...
  public validarEvento(
    ...
    if (
      ...
      if (!this.validatorService.required(data['programa_educativo'])) {
        error['programa_educativo'] =
          'Debes seleccionar un programa educativo cuando el público objetivo incluye estudiantes';
      }
    }
    ...
    if (!this.validatorService.required(data['cupo_maximo'])) {
      ...
      if (cupo < 1) {
        ...
        error['cupo_maximo'] =
          'El cupo no puede exceder 999 (máximo 3 dígitos)';
      }
    }
  ...
  public obtenerResponsables(): Observable<any> {
    ...
    const maestros$ = this.http.get<any>(`${this.apiUrl}/lista-maestros/`, {
      headers,
    });
    const admins$ = this.http.get<any>(`${this.apiUrl}/lista-admins/`, {
      headers,
    });
  ...
  public obtenerEventoPorID(idEvento: number): Observable<any> {
    ...
    return this.http.get<any>(`${this.apiUrl}/eventos/?id=${idEvento}`, {
      headers,
    });
  }
  ...
  public registrarEvento(data: any): Observable<any> {
    ...
    if (datosFormato.hora_inicio) {
      datosFormato.hora_inicio = this.convertirHoraA24(
        datosFormato.hora_inicio
      );
    }
    ...
    console.log(
      'responsable_id enviado:',
      datosFormato.responsable_id,
      'Tipo:',
      typeof datosFormato.responsable_id
    );
    console.log('Headers:', headers);

    return this.http.post<any>(`${this.apiUrl}/eventos/`, datosFormato, {
      headers,
    });
  }
  ...
  public eliminarEvento(idEvento: number): Observable<any> {
    ...
    return this.http.delete<any>(`${this.apiUrl}/eventos/?id=${idEvento}`, {
      headers,
    });
  }
